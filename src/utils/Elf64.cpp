...
string Elf64::to_string(void* elf64_data) const {
    ...
    string MAGIC { (s8)hdr->e_ident[Identification::EI_MAG1], (s8)hdr->e_ident[Identification::EI_MAG2], (s8)hdr->e_ident[Identification::EI_MAG3]};
    result += MAGIC  + "\n";
    result += enum_to_str(hdr->e_ident[Identification::EI_CLASS], "32bit=0x1", "64bit=0x2") + "\n";
    result += enum_to_str(hdr->e_ident[Identification::EI_DATA], "Little endian=0x1", "Big endian=0x2") + "\n";
    result += enum_to_str(hdr->e_ident[Identification::EI_VERSION], "Version 1=0x1") + "\n";
    result += enum_to_str(hdr->e_ident[Identification::EI_OSABI], "SYSV=0x0", "HPUX=0x1", "STANDALONE=0xFF") + "\n";
    result += enum_to_str(hdr->e_ident[Identification::EI_ABIVERSION], "SYSVR3=0x0") + "\n";
    ...
    for (int i = 0; i < hdr->e_shnum*2; i++) {
        ...
        char* section_names = (char*) ((sections_start + hdr->e_shstrndx)->sh_offset + (char*)elf64_data);
        u16 name_offset = esh->sh_name;
        char* section_name = section_names + name_offset;
        result += format("   %. addr: %, len: %, name: %, type: %, flags: %\n", i, esh->sh_addr, esh->sh_size, section_name, type.c_str(), flags.c_str());
