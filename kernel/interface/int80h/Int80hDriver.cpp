...
hardware::CpuState* Int80hDriver::on_interrupt(hardware::CpuState* cpu_state) {
    ...
    case Int80hSysCallNumbers::KILL:
        return task_kill(cpu_state, cpu_state->rbx);; // rcx holds "int signal" but is not used yet
...
hardware::CpuState* Int80hDriver::task_kill(hardware::CpuState* cpu_state, u32 task_id) {
    KernelLog& klog = KernelLog::instance();
    TaskManager& mngr = TaskManager::instance();
    klog.format("[killing task %]\n", task_id);
    return mngr.kill_task_group(cpu_state, task_id);
}
